    ...
    public static int Execute(IEnumerable<string> args)
    {
        ...
        app.Configure(config =>
        {
            ...
            config.AddCommand<AddTaskEntryCommand>("add:task")
                ...
                .WithExample(new[] { "add:task", "the program will prompt you for inputs" })
                .WithExample(new[] { "add:task", "--title [-t] \"Respond to Emails\"", "(Category will default to the first record found)" })
                .WithExample(new[] { "add:task", "--category [-c] personal", "--title [-t] \"Pickup Mail\"" })
                .WithExample(new[] { "add:task", "--category [-c] work", "--title [-t] \"Migrate Database\"", "--note [-n] \"Consult with DBA for access to server\"" });
        });
